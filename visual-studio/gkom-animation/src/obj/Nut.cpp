...
void anim::Nut::initVertices() {
	...
	for (int i = 0; i < 2; i++) {
		...
		for (unsigned int a = 0; a < accuracy; a++) {
			angle = a * 45 * deg2Rad;
...
//			cout << t1 << ":" << t2 << ":" << t3 << ":" << t4 << endl;
	...
	//boki
	unsigned int zJump = accuracy * 2;
	float d1, d2;
	for (unsigned int a = 0; a < accuracy; a++) {
		unsigned int out1 = a * 2, in1 = out1 + 1;
		unsigned int out2 = a * 2 + 2;
		if (out2 == accuracy * 2) out2 = 0;
		unsigned int in2 = out2 + 1;
